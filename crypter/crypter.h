...
namespace seq {
	...
	void encrypt(size_t len, uint64_t* datat, __m256i decade, __m256i football, __m256i deadbeef, __m256i cofeebabe, __m256i declassified, __m256i disclosed, __m256i obsessed, __m256i idea) {
#ifdef USE_VECTORS
	...
	void decrypt(size_t len, uint64_t* datat, __m256i decade, __m256i football, __m256i deadbeef, __m256i cofeebabe, __m256i declassified, __m256i disclosed, __m256i obsessed, __m256i idea) {
#ifdef USE_VECTORS
		for (auto i = 0; i < len; i += 4) {
			auto mm = _mm256_loadu_epi64(&datat[i]);
	...
	// argv for on disk space, ptr for in memory operations
	int Crypter(int argc, char* argv[], std::stringstream* ptr) {
		...
		uint8_t memory_buffer = MEMORY_BUFFER * (ptr != NULL);
		...
		std::string in = memory_buffer != MEMORY_BUFFER ? argv[argc - 2] : "";
		std::string out = memory_buffer != MEMORY_BUFFER ? argv[argc - 1] : "";
		if (argc == 2) {
			in = out;
			auto pather = std::filesystem::current_path();
			if (in.substr(in.length() - 4, 4) == ".seq") {
				pather += "/output.retype";
				crypter = DECRYPT;
				packer = DONTPACK;
			}
			else {
	...
	// prepare vectors for SIMD operations
		...
		if (memory_buffer == MEMORY_BUFFER) {
			const uint64_t sz = argc;
			std::string str(sz, '\0');
			ptr->read(str.data(), sz);
			auto data = str.data();
			...
			decrypt(len, datat, decade, football, deadbeef, cofeebabe, declassified, disclosed, obsessed, idea);
			...
			ptr->str(std::string());
			ptr->write(datafinal, totallenfinal);
			return argc;
		}
		if (packer == DONTPACK) {
			...
			// Same as for SIMD
				...
				auto datat = reinterpret_cast<uint64_t*>(data);
				encrypt(len, datat, decade, football, deadbeef, cofeebabe, declassified, disclosed, obsessed, idea);
